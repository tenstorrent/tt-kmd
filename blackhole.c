...
static u8 __iomem *bh_setup_tlb(struct blackhole_device *bh, u32 x, u32 y, u64 addr) {
...
static u32 noc_read32(struct blackhole_device *bh, u32 x, u32 y, u64 addr) {
	...
	tlb_window = bh_setup_tlb(bh, x, y, addr);
...
static void blackhole_save_reset_state(struct tenstorrent_device *tt_dev) {
	...
	bh->mps = (noc_read32(bh, x, y, PCIE_DBI_ADDR + DEVICE_CONTROL_DEVICE_STATUS) >> MPS_OFFSET) & MPS_MASK;
}
...
static void blackhole_restore_reset_state(struct tenstorrent_device *tt_dev) {
	...
	device_control |= bh->mps << MPS_OFFSET;
