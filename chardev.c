...
int tenstorrent_set_aggregated_power_state(struct tenstorrent_device *tt_dev)
{
	...
	list_for_each_entry(priv, &tt_dev->open_fds_list, open_fd) {
		...
		if (flags_count == 0)
			unspecified_flags_mask = TT_POWER_FLAG_ALL;
		else
...
static int tt_cdev_open(struct inode *inode, struct file *file)
{
	...
	bool legacy_client = !(file->f_flags & O_APPEND);
...
static int tt_cdev_release(struct inode *inode, struct file *file)
{
	...
	if (!tt_dev->detached) {
		struct tenstorrent_set_noc_cleanup cleanup;

		mutex_lock(&priv->mutex);
		cleanup = priv->noc_cleanup;
		mutex_unlock(&priv->mutex);

		if (cleanup.enabled)
			tt_dev->dev_class->noc_write32(tt_dev, cleanup.x, cleanup.y, cleanup.addr,
						       cleanup.data & 0xFFFFFFFF, cleanup.noc);

		if (power_policy) {
			mutex_lock(&priv->mutex);
			// Set all flags to valid (15) and value to 0 (off)
			priv->power_state.validity = TT_POWER_VALIDITY(15, 0);
