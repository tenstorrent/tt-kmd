...
void signal_handler(int signum) {
	signal_count++;
	write(STDOUT_FILENO, ">>> SIGNAL RECEIVED - LONGJMP <<<\n", 35);
...
int main(int argc, char *argv[]) {
	if (argc != 3) {
		...
		std::cerr << "Example: " << argv[0] << " 0 10  (SIGUSR1)\n";
		return 1;
	}
	...
	std::cout << "Daemon starting: PID=" << getpid() 
		  << ", device=" << dev_id 
		  << ", signal=" << signum << "\n";
	...
	while (true) {
		// Install (or reinstall) signal handler
		struct sigaction sa;
		memset(&sa, 0, sizeof(sa));
		sa.sa_handler = signal_handler;
		sigemptyset(&sa.sa_mask);
		sa.sa_flags = SA_NODEFER;  // Don't block the signal during handler execution
