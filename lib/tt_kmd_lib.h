...
typedef struct tt_noc_addr_config_t {
    ...
    uint8_t linked;     /**< TODO documentation */
    uint8_t static_vc;  /**< TODO documentation */
...
enum tt_dma_map_flags {
     ...
     *
     * WH: Buffer will not be accessible via the NOC.
     * BH: Buffer is accessible via the NOC; driver picks the address.
     */
    TT_DMA_FLAG_NONE = 0,
 ...
 * host buffer via the NOC. If mapped without a `NOC` flag, this function
 *
 * WH: will fail
 * BH: will return a NOC address derived from the IOVA/PA
 *
 * @param dma DMA handle from `tt_dma_map()`
