...
void VerifyReleaseHandler(const EnumeratedDevice &dev, uint32_t x, uint32_t y, uint64_t addr)
{
    ...
    static constexpr uint32_t initial = 0x0;
    {
        ...
        tenstorrent_noc_write_on_release on_release{};
        on_release.argsz = sizeof(on_release);
        on_release.enabled = true;
        on_release.data = pattern;
        on_release.x = x;
        on_release.y = y;
        on_release.addr = addr;

        if (ioctl(dev_fd.get(), TENSTORRENT_IOCTL_NOC_WRITE_ON_RELEASE, &on_release) != 0)
