...
uint64_t get_bar4_size(const EnumeratedDevice &dev)
{
    ...
    uint32_t count = 16;
    std::vector<std::byte> buf(sizeof(tenstorrent_query_mappings) + count * sizeof(tenstorrent_mapping));
    ...
    if (ioctl(dev_fd.get(), TENSTORRENT_IOCTL_QUERY_MAPPINGS, buf.data()) != 0)
        THROW_TEST_FAILURE("TENSTORRENT_IOCTL_QUERY_MAPPINGS failed.");

    auto *mappings = reinterpret_cast<tenstorrent_mapping*>(buf.data() + offsetof(tenstorrent_query_mappings, out.mappings));
    for (uint32_t i = 0; i < count; i++) {
        if (mappings[i].mapping_id == TENSTORRENT_MAPPING_RESOURCE2_UC)
            return mappings[i].mapping_size;
        if (mappings[i].mapping_id == TENSTORRENT_MAPPING_UNUSED)
            break;
    }

    THROW_TEST_FAILURE("BAR4 (RESOURCE2) not found in mappings");
}
