...
%preun
# Only remove on full uninstall, not upgrade.
# RPM sets \$1 to the number of instances remaining
# after this operation (0 = uninstall, 1 = upgrade).
# During upgrade the old %preun runs after the new
# %post, so unconditional removal would nuke the
