...
//
// Keraunos test tool - exercise TENSTORRENT_IOCTL_NOC_READ / _NOC_WRITE.
//
// On Keraunos, the generic NOC ioctl address is a flat 52-bit system physical
// address (SPA); x, y, and noc are unused and must be zero. The kernel reserves
// and programs a TLB entry, so userspace does no mmap or BAR/TLB math.
//
// To Compile:
...
//   ./keraunos_read32 --qemu <device_id>                    # sparse-SPA suite
...
//     -q / --qemu   : run deterministic QEMU sparse-SPA ioctl tests
...
#define KERAUNOS_SPA_SIZE (UINT64_C(1) << 52)
...
{
	struct tenstorrent_noc_write io = {0};
...
struct qemu_spa_test {
	const char *name;
	uint64_t spa;
	uint32_t value;
};

static int qemu_read_after_write(int fd, const struct qemu_spa_test *test)
{
	uint32_t got = 0;
	int rc;

	rc = do_write32(fd, test->spa, test->value);
	if (rc) {
		printf("FAIL  %-18s write 0x%013llx: %s\n", test->name,
		       (unsigned long long)test->spa, strerror(-rc));
		return 1;
	}

	rc = do_read32(fd, test->spa, &got);
	if (rc) {
		printf("FAIL  %-18s read  0x%013llx: %s\n", test->name,
		       (unsigned long long)test->spa, strerror(-rc));
		return 1;
	}

	if (got != test->value) {
		printf("FAIL  %-18s SPA 0x%013llx wrote 0x%08x, read 0x%08x\n",
		       test->name, (unsigned long long)test->spa,
		       test->value, got);
		return 1;
	}

	printf("PASS  %-18s SPA 0x%013llx = 0x%08x\n", test->name,
	       (unsigned long long)test->spa, got);
	return 0;
}

static int qemu_expect_read_error(int fd, const char *name, uint64_t spa,
				   int expected_errno)
{
	uint32_t value = 0;
	int rc = do_read32(fd, spa, &value);

	if (rc == -expected_errno) {
		printf("PASS  %-18s SPA 0x%013llx rejected with %s\n", name,
		       (unsigned long long)spa, strerror(expected_errno));
		return 0;
	}

	if (rc)
		printf("FAIL  %-18s SPA 0x%013llx returned %s, expected %s\n",
		       name, (unsigned long long)spa, strerror(-rc),
		       strerror(expected_errno));
	else
		printf("FAIL  %-18s SPA 0x%013llx unexpectedly read 0x%08x\n",
		       name, (unsigned long long)spa, value);
	return 1;
}

static int qemu_expect_write_error(int fd, const char *name, uint64_t spa,
				    int expected_errno)
{
	int rc = do_write32(fd, spa, 0xdecafbad);
	...
	if (rc)
		...
		printf("FAIL  %-18s SPA 0x%013llx write unexpectedly succeeded\n",
		       name, (unsigned long long)spa);
	return 1;
}

static int qemu_expect_unused_fields_error(int fd, const char *name,
					    uint16_t x, uint16_t y,
					    uint8_t noc, int write)
{
	struct tenstorrent_noc_write io = {0};
	int cmd = write ? TENSTORRENT_IOCTL_NOC_WRITE : TENSTORRENT_IOCTL_NOC_READ;
	...
	io.noc = noc;
	io.width = 4;
	io.addr = UINT64_C(0x400000080);
	io.value = 0xdecafbad;

	if (ioctl(fd, cmd, &io) < 0 && errno == EINVAL) {
		printf("PASS  %-18s rejected with %s\n", name, strerror(EINVAL));
		return 0;
	}

	printf("FAIL  %-18s was not rejected with %s\n", name, strerror(EINVAL));
	return 1;
}

static int run_qemu_suite(int fd)
{
	static const struct qemu_spa_test tests[] = {
		{ "low SPA",       UINT64_C(0x0000000400000080), 0x10203040 },
		{ "bit 48 SPA",    (UINT64_C(1) << 48) + 0x80,   0x48524157 },
		{ "bit 51 SPA",    (UINT64_C(1) << 51) + 0x80,   0x51504157 },
		{ "final 52-bit",  KERAUNOS_SPA_SIZE - 4,        0xf52f52f5 },
	};
	int failures = 0;
	size_t i;

	printf("=== QEMU sparse-SPA ioctl tests ===\n");
	for (i = 0; i < sizeof(tests) / sizeof(tests[0]); i++)
		failures += qemu_read_after_write(fd, &tests[i]);

	failures += qemu_expect_read_error(fd, "misaligned SPA",
					  (UINT64_C(1) << 48) + 1, EINVAL);
	failures += qemu_expect_read_error(fd, "out-of-range SPA",
					  KERAUNOS_SPA_SIZE, EINVAL);
	failures += qemu_expect_write_error(fd, "misaligned write",
					   (UINT64_C(1) << 48) + 1, EINVAL);
	failures += qemu_expect_write_error(fd, "out-of-range write",
					   KERAUNOS_SPA_SIZE, EINVAL);
	failures += qemu_expect_unused_fields_error(fd, "read nonzero x",
						    1, 0, 0, 0);
	failures += qemu_expect_unused_fields_error(fd, "read nonzero y",
						    0, 1, 0, 0);
	failures += qemu_expect_unused_fields_error(fd, "read nonzero noc",
						    0, 0, 1, 0);
	failures += qemu_expect_unused_fields_error(fd, "write nonzero x",
						    1, 0, 0, 1);
	failures += qemu_expect_unused_fields_error(fd, "write nonzero y",
						    0, 1, 0, 1);
	failures += qemu_expect_unused_fields_error(fd, "write nonzero noc",
						    0, 0, 1, 1);

	printf("\nQEMU sparse-SPA suite: %s\n", failures ? "FAIL" : "PASS");
	return failures ? 1 : 0;
}
...
static void usage(const char *prog)
{
	...
	fprintf(stderr, "  %s --qemu <device_id>                  # QEMU sparse-SPA suite\n", prog);
...
int main(int argc, char *argv[])
{
	...
	const char *pos[3];
	int npos = 0;
	int force = 0, dump_all = 0, qemu_suite = 0, have_write = 0;
	...
	for (int i = 1; i < argc; i++) {
		...
		if (!strcmp(a, "-f") || !strcmp(a, "--force")) {
			...
			dump_all = 1;
		} else if (!strcmp(a, "-q") || !strcmp(a, "--qemu")) {
			qemu_suite = 1;
	...
	if (qemu_suite && (npos != 1 || force || dump_all || have_write)) {
		fprintf(stderr, "%s: --qemu only accepts a device_id\n", prog);
		usage(prog);
		return 2;
	}
	...
	printf("=== Keraunos Test Tool (NOC_READ / NOC_WRITE) ===\n");
