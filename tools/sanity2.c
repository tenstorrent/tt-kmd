...
//
// Parallel Sanity Check Tool for Tenstorrent Blackhole Devices.
//
// Same checks as sanity.c (NOC coordinate verification + 2MB DMA loopback)
// but runs each device's tests in its own thread for faster execution.
//
// With -n <count>, waits until at least <count> devices are visible.
//
// To Compile:
//  gcc -o sanity2 sanity2.c -lpthread
//
// To Run:
//  ./sanity2             # check whatever is present now
//  ./sanity2 -n 32       # wait for 32 devices, then check all
...
#define NOC_CHECK_DURATION_S 10
...
static int dma_loopback_test(int fd)
{
	...
	uint32_t seed = 0xDEADBEEF ^ (uint32_t)(uintptr_t)buf;
...
static void *device_thread(void *arg)
{
	...
	if (ctx->dev.device_id != PCI_DEVICE_ID_BLACKHOLE) {
		ctx->noc_result = 0;
		ctx->dma_result = 0;
		return NULL;
	}
	...
	char path[PATH_MAX];
	snprintf(path, sizeof(path), "/dev/tenstorrent/%d", ctx->dev.dev_id);

	int fd = open(path, O_RDWR | O_CLOEXEC);
	if (fd < 0)
		return NULL;

	ctx->noc_result = noc_sanity_check(fd);
	ctx->dma_result = dma_loopback_test(fd);
...
int main(int argc, char *argv[])
{
	...
	struct thread_ctx contexts[MAX_DEVICES];
	pthread_t threads[MAX_DEVICES];

	for (int i = 0; i < count; i++) {
		contexts[i].dev = devices[i];
		pthread_create(&threads[i], NULL, device_thread, &contexts[i]);
	}
	...
	for (int i = 0; i < count; i++) {
		...
		int noc_bad = contexts[i].noc_result;
		int dma_bad = contexts[i].dma_result;
