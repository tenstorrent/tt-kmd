...
static void wormhole_cleanup(struct tenstorrent_device *tt_dev) {
	...
	if (wh_dev->reg_iomap != NULL)
		pci_iounmap(wh_dev->tt.pdev, wh_dev->reg_iomap);
}
...
static const u32 TLB_REG_ADDRESS_WIDTHS[NUM_TLB_KINDS] = { 16, 15, 12 };
				...
				int tlb, u64 *regs)
{
	...
	*regs |= (u64)non_address_bits.reg << TLB_REG_ADDRESS_WIDTHS[kind];
...
{
	struct wormhole_device *bh = tt_dev_to_wh_dev(tt_dev);
	u64 regs = 0;
	u32 offset;
	...
	offset = PCI_TLB_CONFIG_OFFSET + tlb * 2 * sizeof(u32);

	if (construct_tlb_config(config, tlb, &regs))
		return -EINVAL;

	iowrite32(regs & 0xFFFFFFFF, bh->reg_iomap + offset);
	iowrite32((regs >> 32) & 0xFFFFFFFF, bh->reg_iomap + offset + sizeof(u32));
